...
Once you have the files, use `read_polecat()` to load them into R:

``` r
# Read all files in a directory into a single data frame
...
The same event can appear in multiple news stories, producing duplicate
rows. Use `deduplicate_polecat()` to remove them:

``` r
dat_deduped <- deduplicate_polecat(dat)
```

Some columns (like `context`) may hold multiple semicolon-separated
values. Use `expand_polecat_col()` to convert to long format:

``` r
# One row per context value
dat_long <- expand_polecat_col(dat, "context")
```

The package also includes information from the [PLOVER
